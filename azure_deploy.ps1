# Deploy Interativo no Azure - Gestor Financeiro
...
if ($HasAccount -ne "s") {
    Write-Host "Por favor, crie uma conta em https://azure.microsoft.com e volte aqui!" -ForegroundColor Red
    exit
}
...
Write-Host "Você pode monitorar em: https://portal.azure.com" -ForegroundColor Gray
Write-Host ""
for ($i = 1; $i -le 6; $i++) {
...
Write-Host ""
$DbUrl = Read-Host "Cole aqui a string de conexão (ou pressione ENTER para usar default)"
if ([string]::IsNullOrEmpty($DbUrl)) {
    $DbUrl = "postgresql://postgres:Senha123!@#@$DbName.postgres.database.azure.com:5432/postgres"
    Write-Host "Usando: $DbUrl" -ForegroundColor Yellow
}
...
Write-Host "  • SECRET_KEY = $(([guid]::NewGuid()).ToString().Replace('-', '').Substring(0, 40))" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
$AppUrl = Read-Host "Cole aqui a URL do seu App (ou ENTER para pular)"
...
Write-Host ""
Write-Host "Referência: Leia AZURE_DEPLOY_MANUAL.md para detalhes" -ForegroundColor Gray
Write-Host ""
